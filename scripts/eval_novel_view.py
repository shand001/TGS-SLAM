...
if __name__=="__main__":
    ...
    if not experiment.config['load_checkpoint']:
        ...
        dst_cfg = os.path.join(results_dir, "config.py")
        # Skip copy when source and destination are identical to avoid SameFileError
        if os.path.abspath(args.experiment) != os.path.abspath(dst_cfg):
            shutil.copy(args.experiment, dst_cfg)
    ...
    if not load_semantics:
        use_one_hot_semantics = False
        ...
        if config['use_wandb']:
            if dataset_config['use_train_split']:
                eval(dataset, params, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
                    ...
                    num_semantic_classes=num_semantic_classes, use_one_hot_semantics=use_one_hot_semantics)
            else:
                eval_nvs(dataset, params, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
                    wandb_run=wandb_run, wandb_save_qual=config['wandb']['eval_save_qual'],
                    mapping_iters=config['mapping']['num_iters'], add_new_gaussians=config['mapping']['add_new_gaussians'],
                    load_semantics=load_semantics, eval_every=config['eval_every'], save_frames=True,
                    use_one_hot_semantics=use_one_hot_semantics, num_semantic_classes=num_semantic_classes)
        else:
            if dataset_config['use_train_split']:
                eval(dataset, params, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
                    mapping_iters=config['mapping']['num_iters'], add_new_gaussians=config['mapping']['add_new_gaussians'],
