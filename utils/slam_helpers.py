...
def params_to_gsplat_inputs(
    params,
    transformed_gaussians,
    use_semantics: bool = False,
    use_one_hot_semantics: bool = False,
    out: dict = None,
    use_rgb_Triplane: bool = False,
    use_sem_Triplane: bool = False,
):
    # log_scales: [N,1] 或 [N,3] -> 统一到各向异性 [N,3]
    log_scales = params['log_scales']
    if log_scales.shape[1] == 1:
        log_scales = log_scales.expand(-1, 3)

    mean  = transformed_gaussians.contiguous()                   # [N,3]
    quat  = F.normalize(params['unnorm_rotations'], dim=-1)  # [N,4], 单位四元数
    scale = torch.exp(log_scales).contiguous()                               # [N,3]
    opac  = torch.sigmoid(params['logit_opacities']).reshape(-1).contiguous()# [N]
    # RGB source
    if use_rgb_Triplane:
        if out is None or ('rgb_colors' not in out):
            raise ValueError("Triplane RGB requested but decoder output missing 'rgb_colors'.")
        color = out['rgb_colors'].contiguous()
    else:
        color = params['rgb_colors'].contiguous()

    # Append semantic features if needed
    if use_semantics:
        if use_one_hot_semantics:
            if use_sem_Triplane:
                if out is None or ('semantic_id' not in out):
                    raise ValueError("Triplane semantic requested but decoder output missing 'semantic_id'.")
                sem_ids = out['semantic_id'].contiguous()
            else:
                sem_ids = params['semantic_id'].contiguous()
            color = torch.cat([color, sem_ids], dim=-1)
        else:
            # Semantic colors always come from per-point params
            sem_cols = params['semantic_colors'].contiguous()
            color = torch.cat([color, sem_cols], dim=-1)

    # [N,3]（你这边是预计算语义颜色）
    # 确保 dtype/device 统一（可选）
    dev   = mean.device
    dtype = torch.float32
    mean, quat, scale, opac, color = [t.to(device=dev, dtype=dtype) for t in (mean, quat, scale, opac, color)]
    return mean, quat, scale, opac, color
